...
**Dual runtime model.** Microsandbox microVMs run workspaces; Docker/Podman run the service tier (LiteLLM + Ollama, plus a small `aip-litellm-db` Postgres — on a private `aip-net` network, trust auth, host port 5442 loopback — that backs LiteLLM's DB-only admin UI/virtual keys; it is the one stateful service-tier piece). Detection (`internal/runtime` + `internal/sandbox`) is parameterized by `GOOS/GOARCH` + a `Prober` so it tests on any host; missing deps → exit 3, missing rootless/virtualization → exit 4. macOS requires **Apple Silicon** (no Intel). S1 egress is ClawPatrol-as-forward-proxy + a default-deny Microsandbox network policy; transparent WireGuard L3 capture is the deferred end-state.
