...
// default provider and points pi's skills/prompts RESOURCE PATHS at the symlinked
// shared pools (pi resolves these paths relative to .pi, where we symlink
// skills/ and prompts/ into the shared .ai-platform pools). Keyless.
// (pi.dev/docs/latest/settings.)
func PiSettings() ([]byte, error) {
	document := map[string]any{
...
func MergePiSettings(existing []byte) ([]byte, error) {
	generated, err := PiSettings()
	if err != nil {
		return nil, err
	}
