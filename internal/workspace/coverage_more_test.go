...
func TestRefreshAgentModelsSelfHealsOrphanedKey(test *testing.T) {
	...
	manager := Manager{
		...
		Served: fakeServedModels{models: []string{"ollama/heal:1"}},
	...
	if !strings.Contains(openCode, "ollama/heal:1") {
...
func TestRefreshAgentModelsSelfHealFailureFallsThrough(test *testing.T) {
	...
	manager := Manager{
		Builder: &fakeBuilder{}, Sandbox: sandbox, Keys: minter,
		Served: fakeServedModels{models: []string{"ollama/fallthrough:1"}},
		Now:    func() string { return "t" },
	}
	...
	openCode := readProjectConfig(test, root, ".opencode", "opencode.json")
	if !strings.Contains(openCode, "ollama/fallthrough:1") {
...
func TestRefreshAgentModelsEmptyServedLeavesUntouched(test *testing.T) {
	...
	primed := Manager{
		...
		Served: fakeServedModels{models: []string{"ollama/keep:1"}},
		Now:    func() string { return "t" },
	}
	if _, err := primed.Start("app"); err != nil {
		test.Fatalf("prime Start: %v", err)
	}
	if openCode := readProjectConfig(test, root, ".opencode", "opencode.json"); !strings.Contains(openCode, "ollama/keep:1") {
	...
	openCode := readProjectConfig(test, root, ".opencode", "opencode.json")
	if !strings.Contains(openCode, "ollama/keep:1") {
