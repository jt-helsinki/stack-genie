...
// This file renders a docker-compose.yml for the service tier as a DEBUG ARTIFACT. It
...
// docker-compose.yml up -d`, then `logs -f <svc>` / `ps` / `restart <svc>` — for easier
...
func ComposeFilePath() (string, error) {
	...
	return filepath.Join(root, "docker-compose.yml"), nil
}
...
// ServicesComposeYAML renders the service-tier docker-compose.yml. bindHost governs the
...
func ServicesComposeYAML(bindHost string) ([]byte, error) {
	...
	header := "# docker-compose.yml for the AI Development Platform service tier — a DEBUG\n" +
