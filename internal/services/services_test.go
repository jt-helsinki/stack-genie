...
// direct ports are internal-only); omlx is host-native but DOES have its own
// direct address+console (its admin panel, on its own fixed OmlxPort — the only
// place models are managed); proxy is the only OTHER direct host port.
...
func TestCoreAndOptionalServiceNames(test *testing.T) {
	wantCore := []string{"omlx", "presidio", "valkey", "redisinsight", "headroom", "litellm", "proxy", "dns"}
...
func TestImageKeysMatchServiceImageKeys(test *testing.T) {
	cases := map[string][]string{
		"omlx":     {}, // host-native — no image keys
