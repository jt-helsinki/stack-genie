...
// fails setup. The interactive flag only gates whether the "catalog unavailable"
// note is printed (nothing is prompted for either way).
//
// hardware bring-up: the live SyncModels round-trip runs only against a running
// aip-litellm.
func syncInitialModels(em *output.Emitter, interactive bool) {
	cat, err := keysCatalogLoader()
	if err != nil {
		// No catalog (offline + no saved copy): nothing to sync against.
		if interactive {
			_, _ = fmt.Fprintf(em.Err, "%s\n", ui.Muted.Render("Model catalog unavailable ("+err.Error()+") — skipping the initial model sync."))
		}
		return
	}
