...
- **Workspace microVM / agent CLI** — hardware-isolated per-project sandbox (opencode · pi · claude · codex · gemini); holds only a scoped LiteLLM virtual key, with egress defaulting to "public" (DNS-audited, re-lockable to deny). Every OS base bakes in Git, the GitHub CLI, the latest **Python 3**, **uv** (Astral's package/tool manager), and **Graphify** (PyPI `graphifyy`, CLI `graphify`, installed via `uv tool install` with all extras except the region/DB-specific ones); each selected agent CLI registers Graphify with itself (`graphify install [--platform <cli>]`). Python is baked in, not a `--stacks` option — the selectable stacks are `go · node · rust · java · maven · deno`.
...
- **LiteLLM (`aip-litellm`, internal-only)** — the model router and the enforcement point for always-on guardrails (secret masking + a destructive tool-call firewall); holds the real provider keys. Its admin UI is reached only through nginx (`/llm` or the `litellm.<domain>` vhost), never published directly.
